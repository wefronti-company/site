...
void main(){
  ...
  if (uEdgeFade > 0.0) {
    ...
    // Apenas fade nas laterais e embaixo, não no topo
    float fadeLeft = smoothstep(0.0, uEdgeFade, norm.x);
    float fadeRight = smoothstep(0.0, uEdgeFade, 1.0 - norm.x);
    float fadeBottom = smoothstep(0.0, uEdgeFade, norm.y);
    float fade = min(min(fadeLeft, fadeRight), fadeBottom);
    M *= fade;
  }
...
const AnimatedGridBackground: React.FC<PixelBlastProps> = ({
  ...
  const finalColor = color || (isDark ? '#6b7280' : '#e5e7eb');
