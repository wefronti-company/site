...
import { getClienteByEmail, createClienteConta } from '../../../lib/clientDb';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (checkRateLimit(req, RATE_LIMIT_MS, 'cliente-registro')) {
  ...
  const existente = await getClienteByEmail(email);
  if (existente) {
  ...
  const cliente = await createClienteConta(nomeCompleto, email, hash);
  if (!cliente) {
